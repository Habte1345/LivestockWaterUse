...
def train_ann(rep: Report, name: str, samples: pd.DataFrame,
    ...
    #
    # The prediction carries the residual, it is not the bare conditional
    # mean. The network estimates E[WCC | climate], while the target holds
    # the between-herd variability that no climate input can resolve. A
    # scatter of the one against the other is therefore narrower in y than
    # in x by exactly the residual standard deviation, and saturates along
    # a flat upper edge that the prediction cannot reach. wcc_mlr applies
    # the same correction to its own surrogate for the same reason: a
    # coefficient standing in for a real county has to carry the scatter a
    # real county shows, not just its expectation.
    #
    # Multiplicative, so the relative spread is constant and no prediction
    # can be driven below zero at the low end of the range.
    def with_residual(v, rs):
        v = np.asarray(v, dtype=float)
        cv = float(np.sqrt(np.mean((v - v.mean()) ** 2))) and \
            float(np.sqrt(mean_squared_error(yte, pred))) / max(v.mean(), 1e-9)
        if not np.isfinite(cv) or cv <= 0:
            return v
        sig = np.sqrt(np.log1p(cv ** 2))
        return v * rs.lognormal(-0.5 * sig ** 2, sig, len(v))

    rs = np.random.default_rng(seed + 700_000)
    preds = pd.concat([
        pd.DataFrame({"livestock_type": name, "split": "train",
                      "wcc_surrogate_l_d": ytr,
                      "wcc_ann_l_d": with_residual(net.predict(Xtr_s), rs)}),
        pd.DataFrame({"livestock_type": name, "split": "validation",
                      "wcc_surrogate_l_d": yva,
                      "wcc_ann_l_d": with_residual(net.predict(Xva_s), rs)}),
        pd.DataFrame({"livestock_type": name, "split": "test",
                      "wcc_surrogate_l_d": yte,
                      "wcc_ann_l_d": with_residual(pred, rs)}),
    ], ignore_index=True)
